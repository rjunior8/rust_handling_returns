...
mod tests {
    ...
    #[test]
    fn test_get_user_2_when_pass() {
        assert_eq!(Ok(EXISTING), my_project::get_user_2(EXISTING));
    }

    #[test]
    fn test_get_user_2_when_fail() {
        assert_eq!(Err("Couldn't find user!".to_string()), my_project::get_user_2(NONEXISTENT));
    }

    #[test]
    fn test_get_user_using_question_mark_when_pass() {
        let user = my_project::get_user_using_question_mark(EXISTING);
        assert_eq!(Ok(EXISTING), user);
    }

    #[test]
    fn test_get_user_using_question_mark_when_fail() {
        let user = my_project::get_user_using_question_mark(NONEXISTENT);
        assert_eq!(Err(String::from("Invalid user!")), user);
    }
